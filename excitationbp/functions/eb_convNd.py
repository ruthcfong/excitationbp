...
def eb_conv1d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    f = EBConvNd(_single(stride), _single(padding), _single(dilation), False, _single(0), groups)
    if bias is None:
        return f(input, weight)
    return f(input, weight, bias)

def eb_conv2d(input, weight, bias=None, stride=1, padding=0, dilation=1,groups=1):
    f = EBConvNd(_pair(stride), _pair(padding), _pair(dilation), False, _pair(0), groups)
    if bias is None:
        return f(input, weight)
    return f(input, weight, bias)

def eb_conv3d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    f = EBConvNd(_triple(stride), _triple(padding), _triple(dilation), False, _triple(0), groups)
    if bias is None:
        return f(input, weight)
    return f(input, weight, bias)